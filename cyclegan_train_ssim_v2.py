...
from cyclegan_model import build_generator_resnet_9blocks,build_gen_discriminator,patch_discriminator
...
def train(filename_A='/media/root/4c3b1a1e-e2bc-42fe-aeff-321d032e5c70/tensorflow/Multi_center/cyc data/chase/tf record/chase.tfrecords',
    ...
    with tf.variable_scope("Model") as scope:
        ...
        rec_A = patch_discriminator(input_A, "d_A")
        rec_B = patch_discriminator(input_B, "d_B")
        # patch_rec_A = patch_discriminator(input_A, "patch_A")
        # patch_rec_B = patch_discriminator(input_B, "patch_B")

        scope.reuse_variables()

        fake_rec_A = patch_discriminator(fake_A, "d_A")
        fake_rec_B = patch_discriminator(fake_B, "d_B")
        # patch_fake_rec_A = patch_discriminator(fake_A, "patch_A")
        # patch_fake_rec_B = patch_discriminator(fake_B, "patch_B")
        cyc_A = build_generator_resnet_9blocks(fake_B, "g_B")
        cyc_B = build_generator_resnet_9blocks(fake_A, "g_A")

        scope.reuse_variables()

        fake_pool_rec_A = patch_discriminator(fake_pool_A, "d_A")
        fake_pool_rec_B = patch_discriminator(fake_pool_B, "d_B")
        # patch_fake_pool_rec_A = patch_discriminator(fake_pool_A, "patch_A")
        # patch_fake_pool_rec_B = patch_discriminator(fake_pool_B, "patch_B")
        ...
        rec_A_summ = tf.summary.image("cyc_a", cyc_A)
        rec_B_summ = tf.summary.image("cyc_b", cyc_B)
    ...
    with tf.variable_scope("loss") as scope:
        ...
        imB = tf.squeeze(input_B)
        imA = tf.squeeze(input_A)
        imfA = tf.squeeze(fake_A)
        imfB = tf.squeeze(fake_B)

        ssim_A = 1-tf.image.ssim(imB, imfA, max_val=255)
        ssim_B = 1-tf.image.ssim(imA, imfB, max_val=255)

        disc_loss_A = tf.reduce_mean(tf.squared_difference(fake_rec_A, 1))
        # patch_disc_loss_A = tf.reduce_mean(tf.squared_difference(patch_fake_rec_A, 1))

        disc_loss_B = tf.reduce_mean(tf.squared_difference(fake_rec_B, 1))
        # patch_disc_loss_B = tf.reduce_mean(tf.squared_difference(patch_fake_rec_B, 1))

        # g_loss_A = cyc_loss * 10 + disc_loss_B
        # g_loss_B = cyc_loss * 10 + disc_loss_A
        g_loss_A = cyc_loss * 10 + disc_loss_B + ssim_B
        g_loss_B = cyc_loss * 10 + disc_loss_A + ssim_A
        # g_loss_A = cyc_loss * 10 + disc_loss_B + ssim_B*10
        # g_loss_B = cyc_loss * 10 + disc_loss_A + ssim_A*10
        ...
        # patch_d_loss_A = (tf.reduce_mean(tf.square(patch_fake_pool_rec_A)) + tf.reduce_mean(tf.squared_difference(patch_rec_A, 1))) / 2.0
        # patch_d_loss_B = (tf.reduce_mean(tf.square(patch_fake_pool_rec_B)) + tf.reduce_mean(tf.squared_difference(patch_rec_B, 1))) / 2.0
        ...
        # patch_A_vars = [var for var in model_vars if 'patch_A' in var.name]
        # patch_B_vars = [var for var in model_vars if 'patch_B' in var.name]

        # patch_A_trainer = optimizer.minimize(patch_d_loss_A, var_list=patch_A_vars)
        # patch_B_trainer = optimizer.minimize(patch_d_loss_B, var_list=patch_B_vars)
        ...
        # patch_A_loss_summ = tf.summary.scalar("patch_A_loss", patch_d_loss_A)
        # patch_B_loss_summ = tf.summary.scalar("patch_B_loss", patch_d_loss_B)
    ...
    with tf.Session() as sess:
        ...
        chkpt_fname = tf.train.latest_checkpoint('/media/root/4c3b1a1e-e2bc-42fe-aeff-321d032e5c70/tensorflow/Multi_center/cyc data/model/')
        ...
        for epoch in range(sess.run(global_step),epochs):
            ...
            for ptr in range(0, max_images):
                ...
                writer.add_summary(summary_str, epoch * max_images + ptr)
                # Optimizing the patch_D_A network

                # _,summary_str = sess.run([patch_A_trainer, patch_A_loss_summ],
                #                           feed_dict={input_A: A_input, input_B: B_input,
                #                                      lr: curr_lr, fake_pool_A: fake_A_temp1})
                # writer.add_summary(summary_str, epoch * max_images + ptr)
                # # Optimizing the patch_D_A network
                # _,summary_str = sess.run([patch_B_trainer,patch_B_loss_summ],feed_dict={input_A: A_input, input_B: B_input,lr: curr_lr, fake_pool_B: fake_B_temp1})
                # writer.add_summary(summary_str, epoch * max_images + ptr)
                ...
                # patch_D_loss_A = sess.run([patch_d_loss_A],feed_dict={input_A: A_input, input_B: B_input, lr: curr_lr,fake_pool_A:fake_A_temp1,fake_pool_B:fake_B_temp1})
                # patch_D_loss_B = sess.run([patch_d_loss_B],feed_dict={input_A: A_input, input_B: B_input, lr: curr_lr,fake_pool_A:fake_A_temp1,fake_pool_B:fake_B_temp1})
